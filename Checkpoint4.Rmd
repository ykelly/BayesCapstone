...
library(ggplot2)
library(lubridate)
ggplot(data=quakesSince1950, aes(x = hour(hms(time_of_Quake)))) + geom_histogram(color="white") +
  xlab("Hour of Day") + ylab("Number of Earthquakes")
...
ggplot(data=cleanQuakesFin, aes(x=mag, y=-depth)) + geom_point() + 
  geom_abline(slope = 4.424, intercept = -58.374, col = "blue")
lm(depth ~ mag, data=cleanQuakesFin)
```
...
depth_model <- "model{
  ...
  for (i in 1:length(x)) {
    ...
    x[i] ~ dnorm(-58.374+4.424*m[i], tau)
  }
...
depth_jags <- jags.model(textConnection(depth_model),
                        data = list(x=cleanQuakesFin$depth),
                        inits = list(.RNG.name = "base::Wichmann-Hill", .RNG.seed = 1989))

#simulate a sample from the posterior
depth_sim <- coda.samples(depth_jags,
                        variable.names = c("tau"), n.iter=10000)

#store the samples in a data frame:
depth_samples <- data.frame(steps=c(1:10000), depth_sim[[1]])
head(depth_samples)
```
