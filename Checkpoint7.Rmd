...
library(ggplot2)
ggplot(data=regionClean, aes(x=region, y=log(mag))) + geom_boxplot() + xlab("Region") + ylab("Log of Magnitude")
```
...
```{r}
set.seed(1997)
tToQpred <- rep(0,10000)

tToQpred[1] <- rexp(1, rate = exp(pois_rate_samples2[1,]$beta0 + pois_rate_samples2[1,]$beta1.2. + pois_rate_samples2[1,]$beta2*2020 + pois_rate_samples2[1,]$beta3.2.*2020))

# REMEMBER TO ACCOUNT FOR LOG
 for (i in 1:10000) {
   tToQpred[i] <- rexp(1, rate=exp(pois_rate_samples2[i,]$beta0 + pois_rate_samples2[i,]$beta1.2. + pois_rate_samples2[i,]$beta2*2020 + pois_rate_samples2[i,]$beta3.2.*2020))
}
...
ggplot(data=as.data.frame(tToQpred), aes(x=tToQpred)) + 
