...
```{r}
# Take 10,000 samples from exponential - w. Aleutians, 2018
set.seed(1997)
...
tToQAleut2018_20 <- rep(0,20000)

 for (i in 1:20000) {
   tToQAleut2018_20[i] <- rexp(1, rate=exp(pois_rate_samples2[i,]$beta0 + pois_rate_samples2[i,]$beta1.2. + pois_rate_samples2[i,]$beta2*2018 + pois_rate_samples2[i,]$beta3.2.*2018))
}

# Visualize distribution for time til next large quake:
ggplot(data=as.data.frame(tToQAleut2018_20), aes(x=tToQAleut2018_20)) + 
  geom_histogram(boundary=0, color="white", aes(y=..density..)) + xlab("Years to Next Large Quake") + 
  ylab("Density") + lims(x=c(0,10), y=c(0,1.5)) + ggtitle("Time to Next Large Earthquake, Western Aleutian Islands 2018")

# Chance of having to wait less than 1 year until the next large quake
mean(tToQAleut2018_20 < 1)
```
...
map1 <- ggmap(centerLA) + geom_point(data=Quakes2000, aes(x = longitude, y =latitude, color=mag), size = 0.5) +
  scale_color_gradient(low="lightsalmon", high="darkred") + xlab("") + ylab("")
...
```{r}
ggplot(data=quakesAbove7, aes(x=Year_)) + geom_histogram(color="white", binwidth = 1) +
  xlab("Year") + ylab("Count") + ggtitle("Number of Large Earthquakes 1900-2016")
```
...
world <- ggplot() +
  borders("world", regions = ".", colour = "gray85", fill = "gray80") +
  theme_map() 
...
ghost_points_ini <- tibble(
  Year_ = as.Date('1900-01-01'),
  longitude = 0, latitude = 0)

ghost_points_fin <- tibble(
  Year_ = as.Date('2016-01-24'),
  longitude = 0, latitude = 0)
...
map <- world +
  geom_point(aes(x = longitude, y = latitude, 
                 frame = Year_,
                 cumulative = TRUE),
             data = quakesAbove7, colour = 'purple', alpha = .5) +
  geom_point(aes(x = longitude, y = latitude, # this is the init transparent frame
                 frame = Year_,
                 cumulative = TRUE),
             data = ghost_points_ini, alpha = 0) +
  geom_point(aes(x = longitude, y = latitude, # this is the final transparent frames
                 frame = Year_,
                 cumulative = TRUE),
...
# Basic scatter plot
#mapping <-  
p <- ggplot(quakesAbove7, mapping = aes(x = longitude, y = latitude, 
               size = mag, color = "red",
               frame = Year_)) +
  geom_point()# +
  #scale_x_log10()
