...
CreateHours <- function(Qdata){
  #Takes a dataset as an input and adds on a time_til_Quake variable that contains the number of hours prior to or after the biggest quake in that dataset; For example if the largest quake occurred on January 5, 1988 at 6 pm, then a quake that occurred in the same region on January 4, 1988 at 5pm would be assigned time_til_Quake = 25. It will be negative if it is an aftershock.
      ...
      if(Qdata$Year_[[i]] == QuakeYear){
        if(Qdata$Month_[[i]] == QuakeMonth){
          if(Qdata$Day_[[i]] == QuakeDay){
            ...
            time_til_Quake[i] = QuakeHour-newhour
        }
      }
    }
    }
