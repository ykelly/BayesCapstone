...
```{r}
# Cutting longitude into 12 categorical ranges
range(quakesAbove7$longitude)
longCategories <- cut(quakesAbove7$longitude, 12)
head(longCategories)
quakesAbove7$LongCat = longCategories

# Grouping earthquakes by year and longitude
library(dplyr)
groupedQuakes <- quakesAbove7 %>%
  group_by(Year_, LongCat) %>%
  summarise(medianMag = median(mag), count = n())
groupedQuakes
```

make a column/vector of mean lambda[i] values from this rjags; then use those values to make lambda [i] in the rjags exponential version; aka different rates of quakes for different longitude groups &/or year groups


```{r}
# Rjags for Poisson regression - predicting rate of earthquakes per year lambda
...
pois_rate_jags <- jags.model(textConnection(pois_rate_model),
                        data = list(numQuakes = groupedQuakes$count, lon = groupedQuakes$LongCat, year = groupedQuakes$Year_),
                        inits = list(.RNG.name = "base::Wichmann-Hill", .RNG.seed = 1989))

#simulate a sample from the posterior
pois_rate_sim <- coda.samples(pois_rate_jags,
                        variable.names = c("beta0", "beta1", "beta2", "lambda"), n.iter=10000)
